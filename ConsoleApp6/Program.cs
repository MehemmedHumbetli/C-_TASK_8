public class Debtor
{
    ...
    {
        return $"{FullName} - {BirthDay.ToShortDateString()} - +994 {Phone} - {Email} - {Debt}";
    }
}
...
class Program
{
    static List<Debtor> debtors = new List<Debtor> {
            ...
            new Debtor("Pamela H. Beauchamp", DateTime.Parse("November 20, 1959"), "801-559-6377", "PamelaHBeauchamp@jourrapide.com", "3239 Tori Lane Salt Lake City, UT 84104", 6588)
        };

    static void Main(string[] args)
    {
        //1
        //debtors.Where(p=> p.Email.EndsWith("rhyta.com") || p.Email.EndsWith("dayrep.com")).ToList().ForEach(p => Console.WriteLine(p));

        //2
        //debtors.Where(p => p.BirthDay.Year > 1988 && p.BirthDay.Year < 1998 ).ToList().ForEach(p => Console.WriteLine(p));

        //3
        //debtors.Where(p => p.Debt < 5000).ToList().ForEach(p => Console.WriteLine(p));

        //4
        /*debtors.Where(p => p.FullName.Length > 18)
            .Where(x => x.Phone.Count(p => p == 7) == 2)
            .ToList().ForEach(p => Console.WriteLine(p));*/

        //5
        //debtors.Where(p => p.BirthDay.Month == 12 || p.BirthDay.Month <= 2  ).ToList().ForEach(p => Console.WriteLine(p));

        //6
        /*int total_debtors = debtors.Count();
        int allDebt = debtors.Sum(p => p.Debt);
        debtors.Where(p => allDebt / total_debtors < p.Debt).ToList().ForEach(p => Console.WriteLine($"{p.FullName}"));*/

        //7
        //debtors.Where(p => p.Phone.Count(s => s != 8) == 0).ToList().ForEach(p => Console.WriteLine($"{p.FullName}         {p.BirthDay.Year}         {p.Debt}"));

        //8
        /*debtors.OrderByDescending(p => p.Debt)
               .Take(5)
               .ToList()
               .ForEach(p => Console.WriteLine(p));*/

        //9
        /*int totalDebt = debtors.Sum(p => p.Debt);
        Console.WriteLine($"Total Debt: {totalDebt}");*/

        //10
        /*        debtors.Where(p => p.BirthDay.Year >= 1939 && p.BirthDay.Year <= 1945)
                       .ToList()
                       .ForEach(p => Console.WriteLine(p));
        */

        //11
        /*debtors.Where(p => p.Phone.GroupBy(c => c).All(g => g.Count() == 1))
               .ToList()
               .ForEach(p => Console.WriteLine($"{p.FullName}         {p.Debt}"));*/
        ...
        debtors.Where(p =>
        {
            DateTime birthDayThisYear = new DateTime(today.Year, p.BirthDay.Month, p.BirthDay.Day);
            if (birthDayThisYear < today)
            {
                birthDayThisYear = birthDayThisYear.AddYears(1);
            }
            int monthsUntilBirthday = ((birthDayThisYear.Year - today.Year) * 12) + birthDayThisYear.Month - today.Month;
            return (p.Debt / 500) <= monthsUntilBirthday;
        })
        .ToList()
        .ForEach(p => Console.WriteLine(p));*/
        ...
        //13
        /*  bool ContainsSmile(string name)
        {
            string fullName = name.ToLower();
            return fullName.Contains('s') && fullName.Contains('m') && fullName.Contains('i') && fullName.Contains('l') && fullName.Contains('e');
        }

        debtors.Where(p => ContainsSmile(p.FullName))
               .ToList()
               .ForEach(p => Console.WriteLine(p));*/
